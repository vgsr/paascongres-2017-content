...
if ( ! class_exists( 'Paco2017_Embed' ) ) :
/**
 * Enable oEmbeds in plugin contexts.
 *
 * Extends WP_Embed class for use with this plugin.
 *
 * @see WP_Embed
 * @see BP_Embed
 *
 * @since 1.1.0
 */
class Paco2017_Embed extends WP_Embed {
	...
	public function __construct() {
		...
		/**
		 * Filters the Paco2017 Content oEmbed setup.
		 *
		 * @since 1.1.0
		 *
		 * @param Paco2017_Embed $this Current instance of the Paco2017_Embed. Passed by reference.
		 */
		do_action_ref_array( 'paco2017_setup_oembed', array( $this ) );
	}
	 ...
	 * enabled, then the URL will be passed to {@link Paco2017_Embed::parse_oembed()}
	 * for oEmbed parsing.
	 *
	 * @since 1.1.0
	 *
	...
	public function shortcode( $attr, $url = '' ) {
		 ...
		 *
		 * @since 1.1.0 Default status of oEmbed discovery has been switched
		 *              to true in WordPress 4.4
		 *
		...
		/**
		 * Filters the embed context ID.
		 *
		 * @since 1.1.0
		 *
		 * @param int $value Value of zero.
		 */
		$context_id = apply_filters( 'paco2017_embed_context_id', 0 );

		return $this->parse_oembed( $context_id, $url, $attr, $rawattr );
	}

	/**
	 * Base function so we can parse links to be embedded.
	 *
	 * View an example to add support in {@link paco2017_partner_adv_embed()}.
	 *
	 * @since 1.1.0
	 *
	...
	public function parse_oembed( $id, $url, $attr, $rawattr ) {
		...
		if ( $id ) {
			 ...
			 */
			$cache = apply_filters( 'paco2017_embed_get_cache', $cache, $id, $cachekey, $url, $attr, $rawattr );
			...
			if ( ! empty( $cache ) ) {
				 ...
				 */
				return apply_filters( 'paco2017_embed_oembed_html', $cache, $url, $attr, $rawattr );
			...
			// If no cache, ping the oEmbed provider and cache the result.
				 ...
				 * @param int    $id       ID to do the caching for.
				 */
				do_action( 'paco2017_embed_update_cache', $cache, $cachekey, $id );

				// If there was a result, return it.
				if ( $html ) {

					/** This filter is documented in includes/classes/class-paco2017-embed.php */
					return apply_filters( 'paco2017_embed_oembed_html', $html, $url, $attr, $rawattr );
				}
			}
		}
